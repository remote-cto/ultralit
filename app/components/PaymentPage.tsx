...
const PaymentPage = () => {
    ...
    try {
      ...
      if (subscriptionRes.ok) {
        ...
        if (subscriptionData.subscription) {
          ...
          if (subscriptionData.subscription.status === 'active' && subscriptionData.subscription.is_active) {
  ...
  const convertPlanToDisplayFormat = (plan: Plan) => {
    ...
    return {
      ...
      buttonColor: planColors[plan.name as keyof typeof planColors] || 'from-gray-500 to-gray-600',
  ...
  const getModifiedPlans = () => {
    ...
    return displayPlans.map(plan => {
      ...
      const isLowerTier = 
        ...
        (subscriptionStatus.plan_name === "Students" && plan.name === "Free Trial");
  ...
  const handlePlanSelection = async (plan: any) => {
    ...
    try {
      ...
      if (hasActiveSubscription && subscriptionStatus) {
        const confirmMessage = plan.amount > subscriptionStatus.amount 
      ...
      if (plan.amount === 0 || plan.is_trial) {
        ...
        if (response.ok && data.success) {
          alert(hasActiveSubscription ? 'Plan changed successfully!' : 'Free trial activated successfully!');
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-yellow-50">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-600">
            {isCheckingSubscription ? "Checking your subscription status..." : 
             isLoadingPlans ? "Loading payment plans..." : "Loading..."}
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-yellow-50 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          {subscriptionStatus && (
            <div className={`${
              hasActiveSubscription ? 'bg-green-100 border-green-300' : 'bg-orange-100 border-orange-300'
            } rounded-xl p-6 mb-6 max-w-3xl mx-auto`}>
              <div className="flex items-center justify-between">
                <div className="text-left">
                  <p className={`${hasActiveSubscription ? 'text-green-800' : 'text-orange-800'} text-lg font-semibold`}>
                    Current Plan: <strong>{subscriptionStatus.plan_name}</strong>
                  </p>
                  <div className="mt-2 space-y-1">
                    <p className={`${hasActiveSubscription ? 'text-green-700' : 'text-orange-700'} text-sm`}>
                    ...
                    {subscriptionStatus.next_renewal_date && (
                      <p className={`${hasActiveSubscription ? 'text-green-700' : 'text-orange-700'} text-sm`}>
                        {hasActiveSubscription ? 'Next Renewal' : 'Expired on'}: {new Date(subscriptionStatus.next_renewal_date).toLocaleDateString('en-IN')}
                      </p>
                    )}
                  </div>
                </div>
              ...
              {!hasActiveSubscription && (
                <p className="text-orange-700 text-sm mt-2 font-medium">
                  Your subscription has expired. Please choose a new plan to continue learning.
                </p>
              )}
            </div>
          )}
              ...
              <div className="text-center">
                ...
                >
                  {isProcessing ? (
                    ...
                    subscriptionStatus?.plan_name === plan.name ? "Current Plan" : "Not Available"
                  ) : hasActiveSubscription ? (
                    plan.amount > (subscriptionStatus?.amount || 0) ? "Upgrade Plan" : 
                    plan.amount < (subscriptionStatus?.amount || 0) ? "Downgrade Plan" : "Switch Plan"
                  ) : (
                    plan.is_trial || plan.amount === 0 ? "Start Free Trial" : "Choose Plan"
                  )}
                </button>
              </div>
            </div>
        ...
        {hasActiveSubscription && (
          <div className="mt-12 bg-blue-50 rounded-xl p-6 border border-blue-200">
            <h3 className="text-lg font-semibold text-blue-800 mb-3">💡 For Existing Subscribers</h3>
            <ul className="text-blue-700 space-y-2 text-sm">
              <li>• Upgrade to access more premium features and content</li>
              <li>• Your current subscription will be prorated when you upgrade</li>
              <li>• You can select additional topics anytime from your dashboard</li>
              <li>• Changes take effect immediately after payment confirmation</li>
            </ul>
          </div>
        )}
